  ...
  for (let i = 0; i < my.SLOT_COUNT; i++) {
    ...
    if (i < entries.length) {
      ...
      try {
        ...
        if (photo.audio && slotEl.dataset.audioKey !== key) {
          slotEl.dataset.audioKey = key;
          try {
            ...
            aud.loop = true;
            aud.volume = 0.6;
            aud.play().catch(e => console.log('audio play err', e));
            slotEl._audio = aud;
          } catch (e) {
            console.log('audio fetch err', e);
          }
        }
      } catch (err) {
...
// ── Upload a new face photo + audio (FIFO: remove oldest if at capacity) ──
...
async function add_action() {
  ...
  try {
    ...
    let audioBlob = await audioPromise;
